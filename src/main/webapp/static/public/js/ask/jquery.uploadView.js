	...
	$.fn.uploadView = function(options){
		...
		$(this).each(function(i){
			$(this).change(function(e){
                ...
                var mydate = new Date();
				$(this).addClass('hide').removeClass('js_upFile');
				index++;
                /*判断index是否为八*/
				if (index<=9) {
					$('.uploadImg').append('<input class="js_upFile" type="file" name="cover'+mydate.getTime()+'">');
		...
		var handleFileSelect = function(obj, _w, _h, _type, _size, _callback){
			...
			reader.onload = (function(theFile){
				return function (e) {
					...
					var img = '<div class="fl js-feedimg pr"><div class="js-delete delete "></div> <img src="' + tmpSrc + '"/> </div>';
					//consoleLog(reader, img);
					//thisClosest.find(showBox).show().html(img);
					thisClosest.find(showBox).append(img);
					if (_w && _h) {
                    ...
                    $('.js-delete').on('click',function () {
                        ...
                        $('.uploadImg').find('input').eq(i).remove();
